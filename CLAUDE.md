...
- **Outputs — one file per writer** (they used to collide on `predictions.csv`; do not re-merge them):
  ...
  - `data/predictions.csv` — per **fixture**. **FROZEN — do not regenerate.** Two kinds of column:
    - `p_home_pre`/`p_draw_pre`/`p_away_pre` — **THE FORECAST**. Model output, no overlay. Produced by the model as committed in **`32ffb4c`, 13 June 2026 12:09 +02:00**, and reproduced bit-for-bit (1.11e-16) by `7b9ad72` on 18 Sep. Enforced by `tests/test_frozen_forecast.py`.
