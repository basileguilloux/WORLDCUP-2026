...
- **Outputs — one file per writer** (they used to collide on `predictions.csv`; do not re-merge them):
  ...
  - `data/predictions.csv` — per **fixture**. **FROZEN — do not regenerate.** Two kinds of column:
    - `p_home_pre`/`p_draw_pre`/`p_away_pre` — **THE FORECAST**. Model output, no overlay. Produced by the model as committed in **`32ffb4c`** (author date 13 June 2026 12:09 +02:00), reproduced bit-for-bit (1.11e-16) by `7b9ad72` on 18 Sep. Enforced by `tests/test_frozen_forecast.py`. **Dating caveat:** that commit reached GitHub only on 18 Sep (remote commit date `2026-09-18T13:41:00Z`; the pre-rebase original `f296b05` is unknown to GitHub — "No commit found for SHA"), so **no server-side timestamp attests 13 June**. The repo itself was created `2026-06-13T09:15:41Z`, which dates the repo, not the forecast. Local-only support: `f296b05`'s committer date matches its author date, and the reflog has `HEAD@{2026-06-13 12:09:22}`. The durable guarantee is not the date but the inputs: `results.csv` holds **no result after 11 June**, so the forecast cannot encode any outcome it predicts.
    - `p_*_post` + the per-team news columns — a **team-news annotation added 18 Sep 2026**, moving 8 of 70 fixtures by ≤6.6 pts. `data/team_news.json` has no fetch timestamp, so its **retrieval date cannot be established**. **Never used for evaluation** — nothing in `src/` or `tests/` reads this file, and the README's comparison is scored from `tournament_sim.csv`.
  - **Leakage caveats** (README has the full list): matchday 1 of 11 Jun (Mexico 2-0 South Africa, South Korea 2-1 Czech Republic) is in `results.csv` and feeds the Elo replay; two forecast fixtures (Canada v Bosnia, USA v Paraguay, both 12 Jun) kicked off *before* the 13 Jun commit and should be excluded from scoring; `10_tournament_sim.py` was written in Sep, after the tournament; git author dates are settable — evidence, not proof (`32ffb4c`'s committer date is 18 Sep because of the rebase recorded in the reflog: `rebase (start): checkout WORLDCUP-2026/main` then `rebase (pick)` at 2026-09-18 15:41:00).
    Guards: `run_pipeline.py --news` exits 1; `09_team_news.py --live` exits 1 without `--overwrite-frozen`.
